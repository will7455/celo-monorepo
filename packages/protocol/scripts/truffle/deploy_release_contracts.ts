...
async function handleGrant(releaseGoldConfig: any, currGrant: number) {
  ...
  if (!response.confirmation) {
    console.info(chalk.yellow('Aborting due to user response'))
    process.exit(0)
  }
  ...
  response = await prompts({
    ...
    name: 'confirmation',
    message:
      'About to initialize RG contract at proxy address ' + releaseGoldProxy.address + 'Ok? (y/n)',
  })
  ...
  response = await prompts({
    ...
    name: 'confirmation',
    message:
      'About to transfer the ownership of the underlying RG contract at ' +
      proxiedReleaseGold.address +
      '\nTo the ReleaseGoldMultiSigProxy at ' +
      releaseGoldMultiSigProxy.address +
      ' Ok? (y/n)',
  })
  ...
  response = await prompts({
    ...
    name: 'confirmation',
    message:
      'About to transfer the ownership of the RG proxy at ' +
  ...
  response = await prompts({
    ...
    name: 'confirmation',
    message:
      'About to send the startGold amount ' +
      startGold +
      'cGLD to the beneficiary: ' +
      releaseGoldConfig.beneficiary +
      ' Ok? (y/n)',
  })
